...
const RegisteredTeamModal: React.FC<RegisteredTeamModalProps> = ({
      ...
      switch (confirmModal.type) {
        ...
        case "remove_member":
          if (confirmModal.data?.memberId) {
            if (team.isCodeWarsTeam) {
              await api.delete(
                `/codewars/${team._id}/member/${confirmModal.data.memberId}`,
              );
            } else {
              await eventTeamApi.removeMember(
                team._id,
                confirmModal.data.memberId,
              );
            }
            window.location.reload();
          }
    ...
    try {
      if (team.isCodeWarsTeam) {
        const response = await api.post(
          `/codewars/${team._id}/regenerate-invite`,
        );
  ...
  const handleInviteMember = async () => {
    ...
    try {
      if (team.isCodeWarsTeam) {
        await api.post(`/codewars/${team._id}/invite`, {
  ...
  const currentMember = userEmail
    ? team.members.find((m) => m.user?.email === userEmail)
    : null;
    ...
    <>
      ...
      {isFullView ? (
                ...
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  {isTeamEvent && isLeader && (
                    <div className="flex items-center gap-2 px-3 py-1 bg-[#E8823A]/20 border border-[#E8823A]/50 rounded">
                      ...
                      <span className="text-xs font-bold text-[#E8823A] uppercase">
                        Leader
                      </span>
                    </div>
                  )}
              ...
              {isTeamEvent ? (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                  ...
                  <div className="bg-white/5 border border-white/10 rounded p-4 hover:border-[#33ABB9]/30 transition-colors">
                    <p className="text-[#33ABB9]/60 text-[9px] uppercase tracking-widest mb-1 font-mono">
                      Event
                    </p>
                    ...
                    {isLeader &&
                      (team.currentSize ?? 0) > (team.maxSize ?? 0) && (
                        <div className="mt-2 text-red-400 text-xs bg-red-400/10 border border-red-400/30 px-3 py-1 rounded">
                          Your team is over capacity. Please remove members to
                          get the status to READY.
                        </div>
                      )}
                  </div>
        ...
        <motion.div
          ...
          onClick={() => {
            if (currentMember?.status?.toLowerCase() !== "left")
              setShowModal(true);
          }}
        >
          {/* Background Shape */}
          <div
            className={`absolute inset-0 bg-white/5 border border-white/10 transition-all ${
              currentMember?.status?.toLowerCase() !== "left"
          ...
          <div className="relative p-6 z-10">
            <div className="flex items-center justify-between mb-4">
              <div className="flex-1">
                <h4 className="text-white font-bold text-lg mb-2 group-hover:text-[#33ABB9] transition-colors">
                  {isTeamEvent ? team.name : "Solo Registration"}
                </h4>
                <div className="flex gap-4 text-gray-400 text-sm">
                  <span className="text-[#33ABB9] font-mono text-xs uppercase">
                    {team.eventId.name}
                  </span>
              ...
              <div className="flex items-center gap-3">
                ...
                {currentMember?.status?.toLowerCase() !== "left" && (
                  <ChevronRight
                    className="text-[#33ABB9] group-hover:translate-x-1 transition-transform"
                    size={20}
                  />
                )}
              </div>
                      ...
                      onClick={async (e) => {
                        ...
                        try {
                          if (team.isCodeWarsTeam) {
                            ...
                            await eventTeamApi.inviteMemberByEmail(
                              team._id,
                              inviteEmail,
                            );
                          }
        ...
        {showModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-sm sm:max-w-2xl md:max-w-4xl lg:max-w-5xl max-h-[90vh] flex flex-col mx-4 overflow-hidden"
              style={{
                clipPath:
                  "polygon(16px 0, 100% 0, 100% calc(100% - 16px), calc(100% - 32px) 100%, 0 100%, 0 16px)",
              }}
            >
              ...
              <div className="absolute inset-0 pointer-events-none">
                {/* Background Shape with ClipPath */}
                <div className="absolute inset-0 bg-[#0a0a0a]/95 backdrop-blur-xl border-0 border-white/5 shadow-2xl" />
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
                    ...
                    <div className="bg-white/5 border border-[#33ABB9]/30 p-4 relative overflow-hidden group hover:border-[#33ABB9]/50 transition-colors">
                      ...
                      <div className="relative">
                        ...
                        <p className="text-2xl font-bold text-[#33ABB9] font-mono">
                          {team.currentSize}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="bg-white/5 border border-white/10 p-4 hover:border-[#33ABB9]/30 transition-colors">
                      <p className="text-gray-400 text-[9px] uppercase tracking-widest mb-1 font-mono">
                        Min Required
                      </p>
                    ...
                    {/* Status */}
                    <div
                      className={`bg-white/5 border p-4 transition-colors ${(team.currentSize ?? 0) >= (team.minSize ?? 0) ? "border-green-400/30 hover:border-green-400/50" : "border-yellow-400/30 hover:border-yellow-400/50"}`}
                    >
                      <p className="text-gray-400 text-[9px] uppercase tracking-widest mb-1 font-mono">
                        Status
                      </p>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-6">
                    ...
                    <div className="bg-white/5 border border-[#33ABB9]/30 p-4 relative overflow-hidden group hover:border-[#33ABB9]/50 transition-colors">
                      ...
                      <div className="relative">
                        ...
                        <p className="text-xl font-bold text-[#33ABB9] font-mono">
                          {eventName}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="bg-white/5 border border-white/10 p-4 hover:border-[#33ABB9]/30 transition-colors sm:col-span-2 lg:col-span-3">
                      <p className="text-gray-400 text-[9px] uppercase tracking-widest mb-2 font-mono">
                        Participant
                      </p>
                      {typeof activeMembers[0]?.user === "object" && (
                        <div>
                          <p className="text-white font-bold text-base">
                            {activeMembers[0].user.name}
                          </p>
                          <p className="text-gray-400 text-sm font-mono">
                            {activeMembers[0].user.email}
                          </p>
                        </div>
                      )}
                    </div>
                ...
                {team.eventId?.whatsappGrpLink && (
                  <div className="mb-6 bg-linear-to-r from-green-900/20 to-transparent border-l-2 border-green-500 p-5 relative overflow-hidden">
                    ...
                    <div className="relative">
                      ...
                      <div className="mb-3">
                        <label className="text-green-400/80 text-xs uppercase tracking-wider mb-2 block font-mono">
                ...
                {team.isCodeWarsTeam && team.gfgLink && (
                  <div className="mb-6 bg-linear-to-r from-orange-900/20 to-transparent border-l-2 border-orange-500 p-5 relative overflow-hidden">
                    ...
                    <div className="relative">
                      ...
                      <div className="mb-3">
                        <label className="text-orange-400/80 text-xs uppercase tracking-wider mb-2 block font-mono">
                          Register & compete on GFG platform
                        </label>
                        <div className="flex flex-col sm:flex-row gap-2">
                          <input
                            type="text"
                  ...
                  <div className="mb-6 bg-linear-to-r from-[#184344]/40 to-transparent border-l-2 border-[#33ABB9] p-5 relative overflow-hidden">
                    ...
                    <div className="absolute inset-0 opacity-5">
                      <div
                        ...
                        style={{
                          backgroundImage:
                            "radial-gradient(circle, #33ABB9 1px, transparent 1px)",
                          backgroundSize: "20px 20px",
                        }}
                      />
                    </div>
                  ...
                  <div className="mb-4">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <h4 className="text-xl font-bold text-white font-orbitron uppercase tracking-wider">
                        Team Members
                        <span className="text-[#33ABB9] ml-2">
                          ({activeMembers.length})
                        </span>
                      </h4>
                    </div>
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="relative flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                              <div className="flex items-center gap-3">
                                <div
                                  className={`w-10 h-10 ${isLeaderMember ? "bg-linear-to-br from-[#E8823A] to-[#E8823A]/60 border-2 border-[#E8823A]" : "bg-[#184344]/40 border-2 border-[#33ABB9]/30"} flex items-center justify-center relative shrink-0`}
                                >
                                  {isLeaderMember ? (
                                    <Crown className="w-4 h-4 text-black" />
                                  ) : (
                                    <span className="text-[#33ABB9] font-mono font-bold text-sm">
                                      {index + 1}
                                    </span>
                                  )}
                        ...
                        <div className="space-y-1">
                          {pendingMembers.map((member) => {
                            ...
                            return (
                              <div
                                key={memberUserId}
                                className="bg-yellow-500/10 border border-yellow-500/30 p-3 flex items-center gap-2"
                              >
          ...
          <div className="fixed inset-0 z-60 flex items-center justify-center p-4">
            ...
            >
              ...
              <div
                ...
                style={{
                  clipPath:
                    "polygon(16px 0, 100% 0, 100% calc(100% - 16px), calc(100% - 24px) 100%, 0 100%, 0 16px)",
                }}
              />
              ...
              <div className="relative z-10 p-8">
                ...
                <div className="mb-6">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-2xl font-bold text-white font-orbitron uppercase">
                      Invite Member
                    </h3>
                  </div>
                ...
                {inviteSuccess ? (
                  ...
                  <>
                    <p className="text-gray-400 mb-4 text-sm">
                      Enter email address to send team invitation:
                    </p>
                    <input
                      type="email"
