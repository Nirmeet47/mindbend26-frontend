  ...
  useEffect(() => {
    ...
    if (activeTab === "accommodation" && !fetchedData.accommodation) {
      ...
      api
        ...
        .catch((err) =>
          setError(
            err?.response?.data?.message || "Failed to load accommodation",
          ),
        )
        .finally(() => setLoading(false));
    }
            ...
            {loading && !fetchedData.profile ? (
              <div className="flex justify-center items-center min-h-[50vh]">
                <div className="text-center">
                  <div className="text-[#33ABB9] text-xl mb-4">
                    Loading Profile...
                  </div>
                      ...
                      if (
                        ...
                        setUserData((prev) => ({
                          ...
                          yearOfStudy: user?.year_of_study
                            ...
                            : prev.yearOfStudy,
                          contactNumber:
                            user?.phoneNumber || prev.contactNumber,
                          isProfileCompleted:
                            user?.isProfile_completed || false,
                        }));
                      } else {
                        alert(
                ...
                {!isProfileComplete && (
                  <div className="relative mb-6">
                    ...
                    <div className="relative p-4 text-[#E8823A]">
                      ...
                      Please complete your profile by filling in your college
                      name, year of study, and contact number.
                    </div>
                  </div>
                )}
                ...
                <div>
                  ...
                  {loading && activeTab !== "overview" && (
                    <div className="flex justify-center items-center py-12">
                      <div className="text-center">
                        <div className="text-[#33ABB9] text-lg mb-4">
                          Loading {activeTab}...
                        </div>
                  ...
                  {activeTab === "workshops" && !loading && (
                    <WorkshopsTab workshops={registeredWorkshops} />
                  )}
                  ...
                  {activeTab === "teams" && !loading && (
                    <TeamsTab
                      ...
                      loading={loading}
                      error={error}
                      onAcceptRejectInvite={async (team, action) => {
                        try {
                          if (team.isCodeWarsTeam) {
                            await api.post(
                              `/codewars/${team._id}/invite/respond`,
                              { action },
                            );
                          } else {
                            await api.post(
                              `/events/team/${team._id}/invite/respond`,
                              { action },
                            );
                          }
                        } catch (err: any) {
                        ...
                        try {
                          ...
                          const regularTeams = res.data?.data?.teams || [];
                          const codewarsTeams =
                            res.data?.data?.codewarsTeams || [];
                          const allTeams = [...regularTeams, ...codewarsTeams];
                          setTeams(allTeams);
                          setPendingInvites((prev) =>
                            prev.filter((i) => i.teamId !== team._id),
                          );
                        } catch (err: any) {
                  ...
                  {/* Accommodation Tab */}
                  {activeTab === "accommodation" && !loading && (
                    <AccommodationTab accommodations={accommodations} />
                  )}
                </div>
              </>
            )}
